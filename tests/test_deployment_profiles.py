...
def test_runbook_uses_ssh_key_as_the_only_normal_deployment_path():
    ...
    assert "创建通用云服务器（SSH Key）" in normal_path
    assert "当前服务器可以是阿里云 ECS" in normal_path
    assert "任意云厂商" in normal_path
    ...
    assert "不要在 VPS Shell 内执行下面的 `ssh`" in normal_path
    assert "IdentitiesOnly=yes" in normal_path
    assert "不要把本机私钥复制到 VPS" in normal_path
    assert normal_path.index("在第二个本机窗口验证 deploy 登录和 sudo") < normal_path.index(
        "最后关闭 root、密码和键盘交互登录"
    )
    assert "刷新 Ubuntu 软件包索引" in normal_path
    assert "不会复制任何私钥" in normal_path
    assert "只检查 sshd 配置语法" in normal_path
    assert "无中断重载配置" in normal_path
    assert "避免日志、备份文件名和数据库时间跨时区混乱" in normal_path
...
def test_runbook_moves_password_recovery_out_of_the_normal_path():
    ...
    assert "云厂商控制台" in troubleshooting
    assert "VNC/串口/救援模式" in troubleshooting
    ...
    assert "VPS 视为已失陷" in troubleshooting
...
def test_runbook_explains_firewall_layers_and_port_purposes():
    ...
    assert "配置云安全组/云防火墙" in runbook
    assert "入站 TCP 22" in runbook
    assert "出站" in runbook
    assert "TCP 22" in runbook
    assert "TCP 80" in runbook
    assert "TCP 443" in runbook
    assert "不要对公网开放" in runbook
...
def test_runbook_uses_reproducible_2c2g_update_commands():
    ...
    assert "docker compose $COMPOSE_FILES" in update_section
