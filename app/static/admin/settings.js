...
async function runStorageCleanup(targets, button, actionLabel) {
  ...
  if (!(await confirmAction(`${actionLabel}，删除后不可恢复。`, { title: "清理存储", confirmText: "确认清理" }))) return;
...
async function refreshSettings() {
  try {
    ...
    renderStorageStats(data.storage);
    await refreshAdminAnnouncements().catch((err) => {
      const list = $("#adminAnnouncementList");
      if (list) list.innerHTML = `<p class="muted">公告加载失败：${escapeHtml(err.message)}</p>`;
    });
    return true;
  } catch (err) {
